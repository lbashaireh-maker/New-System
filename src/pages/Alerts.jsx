...
export default function Alerts() {
  ...
  const load = useCallback(() => {
    ...
    api.get(`/alerts?${p}`).then(setAlerts).finally(() => setLoading(false))
  }, [statusF, severityF])
  ...
  const acknowledge = async (alertId) => {
    setActing(true)
    await api.put(`/alerts/${alertId}/acknowledge`, { officer: 'Compliance Officer' })
    load()
    setActing(false)
  }
  ...
  const resolve = async () => {
    setActing(true)
    await api.put(`/alerts/${resolveItem.alertId}/resolve`, { officer: 'Compliance Officer', resolution })
    setResolveItem(null)
    setResolution('')
    load()
    setActing(false)
  ...
  return (
    <div className="space-y-5 fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[['Open', counts.open, '#EF4444'], ['Critical', counts.critical, '#B91C1C'], ['Acknowledged', counts.acknowledged, '#F59E0B'], ['Resolved', counts.resolved, '#10B981']].map(([label, val, color]) => (
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        {loading ? <Spinner /> : alerts.length === 0 ? (
          <EmptyState icon={CheckCircle} title="No alerts" description="No alerts match your current filters." />
        ) : (
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-50">
              {alerts.map(a => (
                <tr key={a._id || a.alertId} className="hover:bg-gray-50 transition-colors">
                  <td className="px-4 py-3"><Badge type={a.severity} /></td>
                  <td className="px-4 py-3">
                    <Link to={`/customers/${a.customerId}`} className="font-medium text-gray-700 hover:underline">
                      {a.customerName}
                    </Link>
                  </td>
      ...
      <Modal isOpen={!!resolveItem} onClose={() => setResolveItem(null)} title="Resolve Alert" size="sm">
        <p className="text-sm text-gray-600 mb-3">
          Resolving alert for <strong>{resolveItem?.customerName}</strong>: {resolveItem?.message}
        </p>
        <label className="block text-xs font-medium text-gray-600 mb-1">Resolution Notes</label>
        <textarea value={resolution} onChange={e => setResolution(e.target.value)} rows={3}
          placeholder="Describe the action taken..."
          className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none mb-4 resize-none" />
        <div className="flex gap-3">
          ...
          <button onClick={resolve} disabled={acting} className="flex-1 px-4 py-2 text-sm rounded-lg text-white font-medium" style={{ backgroundColor: '#EA8923' }}>
            {acting ? 'Saving...' : 'Mark Resolved'}
          </button>
        </div>
      </Modal>

      <ConfirmDialog isOpen={!!deleteId} onClose={() => setDeleteId(null)} onConfirm={deleteAlert}
        title="Delete Alert" message="Permanently remove this alert?" confirmLabel="Delete" danger />
    </div>
  )
}
