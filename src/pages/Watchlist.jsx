...
export default function Watchlist() {
  ...
  const load = useCallback(() => {
    ...
    api.get(`/watchlist${p}`).then(setEntries).finally(() => setLoading(false))
  }, [search])
  ...
  const handleAdd = async e => {
    ...
    setShowAdd(false)
    setForm({ name:'', aliases:'', nationality:'', dob:'', reason:'', listType:'internal' })
    load()
    setSaving(false)
  }
  ...
  const handleScreenAll = async () => {
    ...
    const res = await api.post('/watchlist/screen-all')
    setScreenRes(res)
    setScreening(false)
  }
  ...
  return (
    <div className="space-y-5 fade-in">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <button onClick={handleScreenAll} disabled={screening}
          ...
          {screening ? 'Screening...' : 'Screen All Customers'}
        </button>
      ...
      {screenRes && (
        <div className={`px-4 py-3 rounded-lg text-sm font-medium ${
          screenRes.newMatches.length > 0 ? 'bg-red-50 text-red-700 border border-red-200' : 'bg-green-50 text-green-700 border border-green-200'
        }`}>
          Screened {screenRes.screened} customers.
          {screenRes.newMatches.length > 0
            ? ` ⚠️ ${screenRes.newMatches.length} new match(es): ${screenRes.newMatches.map(m => m.customer).join(', ')}`
            : ' ✅ No new watchlist matches.'}
        </div>
      )}
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        {loading ? <Spinner /> : entries.length === 0 ? (
          <EmptyState icon={ShieldAlert} title="Watchlist is empty" description="Add entries to screen customers against them." />
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100">
                {['Name','Aliases','Nationality','DOB','Reason','List','Added',''].map(h => (
              ...
              {entries.map(e => (
                <tr key={e.entryId} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-xs text-gray-400">
                    {new Date(e.addedAt).toLocaleDateString('en-GB', { day:'2-digit', month:'short', year:'numeric' })}
                  </td>
                  <td className="px-4 py-3">
                    <button onClick={() => setDeleteId(e.entryId)}
      ...
      <Modal isOpen={showAdd} onClose={() => setShowAdd(false)} title="Add Watchlist Entry">
        <form onSubmit={handleAdd} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
              ...
              <input value={form.aliases} onChange={e => set('aliases', e.target.value)}
                placeholder="Alt Name, Other Alias" className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Nationality</label>
              <input value={form.nationality} onChange={e => set('nationality', e.target.value)}
                placeholder="JO, SY..." className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none" />
            </div>
            <div>
            ...
            <div className="col-span-2">
              ...
              <input required value={form.reason} onChange={e => set('reason', e.target.value)}
                placeholder="Money laundering, Terrorism, etc." className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none" />
            </div>
            <div>
          ...
          <div className="flex gap-3 pt-2 border-t border-gray-100">
            ...
            <button type="submit" disabled={saving} className="flex-1 px-4 py-2 text-sm rounded-lg text-white font-medium" style={{ backgroundColor: '#EA8923' }}>
      ...
      <ConfirmDialog isOpen={!!deleteId} onClose={() => setDeleteId(null)} onConfirm={handleDelete}
        title="Remove Entry" message="Remove this entry from the watchlist? Existing alerts will not be automatically cleared." confirmLabel="Remove" danger />
    </div>
  )
}
